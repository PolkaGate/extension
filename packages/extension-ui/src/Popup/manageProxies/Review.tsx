...
export default function Review({ address, api, chain, depositToPay, depositValue, proxies }: Props): React.ReactElement {
  ...
  useEffect(() => {
    ...
    addingLength && setHelperText(t(`You are adding ${addingLength} Prox${addingLength > 1 ? 'ies' : 'y'}`));
    removingLength && setHelperText(t(`You are removing ${removingLength} Prox${removingLength > 1 ? 'ies' : 'y'}`));
    addingLength && removingLength && setHelperText(t(`Adding ${addingLength} and removing ${removingLength} Proxies`));
  }, [proxies, t]);
