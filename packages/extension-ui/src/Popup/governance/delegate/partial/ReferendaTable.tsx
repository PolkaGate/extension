    ...
    <Grid container>
      <Grid container item justifyContent='center'>
        <Typography fontSize='16px' fontWeight={400}>
          {t('Number of Referenda Categories')}
        </Typography>
      </Grid>
      <Grid container sx={{ '>:last-child': { border: 'none' }, bgcolor: 'background.paper', border: '1px solid', borderColor: 'secondary.light', borderRadius: '5px', maxHeight: `${maxTableHeight ?? 270}px`, overflowY: 'scroll', scrollbarWidth: 'none' }}>
        <Grid alignItems='center' container sx={{ borderBottom: '2px solid', borderBottomColor: 'rgba(0,0,0,0.2)', height: '38px', justifyContent: 'space-between', px: '25px' }}>
          <Typography fontSize={20} fontWeight={400} width='fit-content'>{t('Track name')}</Typography>
          <Typography fontSize={20} fontWeight={400} width='fit-content'>{t('Value')}</Typography>
          <Typography fontSize={20} fontWeight={400} width='fit-content'>{t('Multiplier')}</Typography>
        </Grid>
        {delegatedInfo.map((info, index) =>
          <Grid alignItems='center' container key={index} sx={{ borderBottom: '1px solid', borderBottomColor: 'rgba(0,0,0,0.2)', height: '45px', justifyContent: 'space-between', px: '25px' }}>
            <Typography fontSize={16} fontWeight={400} textAlign='left' width='40%'>{t(trackName(info.track) as string)}</Typography>
