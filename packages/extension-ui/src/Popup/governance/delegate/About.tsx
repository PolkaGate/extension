...
export default function About ({ setStep }: Props): React.ReactElement {
  ...
  const handleNext = useCallback(() => {
    ...
    dontDisplay && window.localStorage.setItem('delegate_about_disabled', 'true');
    setStep(1);
  }, [dontDisplay, setStep]);
  ...
  return (
    <Grid container direction='column'>
      ...
      <Grid container item sx={{ bgcolor: 'background.paper', border: 1, borderColor: 'secondary.light', borderRadius: '5px', pb: '15px', px: '15px' }}>
        <Grid container item>
          <UL
            notes={[t('Delegation empowers trusted individuals  to vote on your behalf.')]}
            title={t('What is Vote Delegation?')}
          />
        </Grid>
        <Grid container item>
          <UL
            notes={[
              t('Delegate to experienced voters for better decisions.'),
              t('Stay involved even if you\'re unable to actively vote.')
            ]}
            title={t('Why Consider Vote Delegation?')}
          />
        </Grid>
        <Grid container item>
          <UL
            notes={[
              t('You can give your voting power to another account with a vote multiplier.'),
              t('The duration of your tokens\' lock period depends on the selected multiplier.'),
              t('You can choose to delegate votes in all tracks or specific ones.'),
              t('If you have already voted in a category, it cannot be selected for delegation.')
            ]}
