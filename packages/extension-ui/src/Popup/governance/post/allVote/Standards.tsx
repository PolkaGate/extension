      ...
      <Grid item onClick={openSearchBar} sx={{ cursor: 'pointer', textAlign: 'start', mr: '9px' }} xs>
        {isSearchBarOpen
          ? <InputFilter
            // autoFocus={false}
            onChange={onSearch}
            placeholder={t('🔍 Search by voter address')}
            theme={theme}
            value={searchKeyword ?? ''}
            withReset
          />
        ...
        <Box>
          <Tabs centered onChange={handleTabChange} sx={{ pt: '15px', 'span.MuiTabs-indicator': { bgcolor: 'secondary.light', height: '5px', width: '100%' } }} value={tabIndex}>
            <Tab
              ...
              iconPosition='start'
              label={t('Ayes ({{ayesCount}})', { replace: { ayesCount: filteredVotes?.yes?.length || 0 } })}
            ...
            <Tab
              ...
              iconPosition='start'
              label={t('Nays ({{naysCount}})', { replace: { naysCount: filteredVotes?.no?.length || 0 } })}
            ...
            <Tab
              ...
              iconPosition='start'
              label={t('Abstains ({{abstainsCount}})', { replace: { abstainsCount: filteredVotes?.abstain?.length || 0 } })}
