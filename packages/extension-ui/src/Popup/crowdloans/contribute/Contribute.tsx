...
export default function Contribute({ api, chain, crowdloan, crowdloansId, currentBlockNumber, formatted, minContribution, myContribution, setShowContribute, showContribute = false }: Props): React.ReactElement {
  ...
  useEffect(() => {
    ...
    tx(...feeDummyParams).paymentInfo(formatted as AddressOrPair).then((i) => setEstimatedFee(i?.partialFee)).catch(console.error);

    tx(...maxFeeDummyParams).paymentInfo(formatted as AddressOrPair).then((i) => setEstimatedMaxFee(i?.partialFee)).catch(console.error);
