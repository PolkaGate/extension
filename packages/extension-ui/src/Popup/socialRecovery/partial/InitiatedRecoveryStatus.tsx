        ...
        {lostAccountRecoveryInfo
          ? <>
            ...
            <Typography fontSize='22px' fontWeight={500} pl='15px'>
              {friendsCount !== 'zero'
                ? t(`You initiated a recovery and need ${friendsCount} vouch${friendsCount !== 'one' ? 'es' : ''}.`)
                : t('You initiated a recovery and received all required vouches.')}
            </Typography>
          </>
          : <Skeleton
            height='30px'
                ...
                <Grid alignItems='center' container item justifyContent='center' sx={{ borderLeft: '1px solid', borderLeftColor: '#D5CCD0' }} width='20%'>
                  {initiatedRecovery?.vouchedFriends.includes(String(friend))
                    ? <>
                      ...
                      <Typography fontSize='14px' fontWeight={400}>{t('Done')}</Typography>
                    </>
                    : <>
                      <AccessTimeIcon sx={{ color: theme.palette.success.main, fontSize: '24px', mr: '8px' }} />
                      <Typography fontSize='14px' fontWeight={400}>{t('Waiting')}</Typography>
                    </>
                  }
                </Grid>
              </Grid>
      ...
      {isDelayPassed === false &&
        <Grid alignItems='center' container item sx={{ borderTop: '2px solid', borderTopColor: '#D5CCD0', pt: '12px', mt: '15px' }}>
          <Grid alignItems='center' container item>
            ...
            <Typography fontSize='20px' fontWeight={500}>
              {t('You need to wait for the delay time to pass to be able to withdraw.')}
            </Typography>
          </Grid>
      ...
      {isVouchedCompleted && isDelayPassed === true &&
        <Grid alignItems='center' container item sx={{ borderTop: '2px solid', borderTopColor: '#D5CCD0', pt: '12px', mt: '15px' }}>
          <Grid alignItems='center' container item>
            ...
            <Typography fontSize='20px' fontWeight={500}>
              {t('Now you can proceed to withdraw.')}
            </Typography>
          </Grid>
        </Grid>
      }
        ...
        <Grid alignItems='center' container item sx={{ borderTop: '2px solid', borderTopColor: '#D5CCD0', pt: '12px', mt: '15px' }}>
          <Grid alignItems='center' container item>
            ...
            <Typography fontSize='20px' fontWeight={500} width='92%'>
              {t('The recovery delay time has already elapsed.')}
            </Typography>
          </Grid>
        </Grid>
      }
