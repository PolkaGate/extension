  ...
  return (
    <Grid container item sx={{ display: 'block', px: '10%' }}>
          ...
          {recoveryInfo
            ...
            : <Grid container item sx={{ '> div.belowInput': { m: 0 }, height: '30px', m: 'auto', width: '240px' }}>
              <Warning
                ...
                theme={theme}
              >
                {t('Your account is not recoverable.')}
              </Warning>
            </Grid>
          }
        </Grid>
      ...
      <Typography fontSize='14px' fontWeight={400} py={activeLost ? '10px' : '25px'}>
        {t('Social recovery provides a user-friendly solution for safeguarding crypto assets in case of a lost seed phrase. It involves relying on trusted friends and family to regain access to your account.')}
      </Typography>
      {activeLost &&
        <Grid alignItems='center' container item justifyContent='space-between' pb='15px' pt='10px'>
          <Grid container item sx={{ '> div.belowInput': { m: 0 }, height: '30px', width: '330px' }}>
            <Warning
              ...
              theme={theme}
            >
              {t('Suspicious recovery detected on your account.')}
            </Warning>
          </Grid>
      ...
      <Grid container direction='column' gap='25px' item>
        <RecoveryOptionButton
          ...
          description={
            recoveryInfo
              ? t('Click to view your selected trusted friend accounts and your account recovery settings.')
              : t('Select trusted friends\' accounts and configure details such as a recovery threshold and a delay to enable account recovery.')
          }
          icon={
            <MakeRecoverableIcon
              color={iconColors}
          ...
          onClickFunction={recoveryInfo
            ...
            : goToMakeRecoverable}
          title={recoveryInfo
            ? t('Check Recoverability Details')
            : t('Make Account Recoverable')}
        />
        <RecoveryOptionButton
          activeLost={activeLost}
          description={
            activeRescue || activeProxy
              ? t('Since you\'ve already initiated the recovery process for a lost account, you can now review the recovery process details and decide on any further actions.')
              : t('If you\'ve lost a recoverable account, you can begin the process of rescuing it from here.')
          }
          icon={
            <RescueRecoveryIcon
              color={iconColors}
          ...
          title={activeRescue || activeProxy
            ? t('You Initiated a Recovery. Check Status')
            : t('Rescue a Lost Account')}
        />
        <RecoveryOptionButton
          activeLost={activeLost}
          description={t('You can assist others in recovering their lost accounts if they\'ve designated your account as a trusted friend during the account recovery setup.')}
          icon={
            <VouchRecoveryIcon
              color={iconColors}
              height={60}
          ...
          title={t('Vouch Recovery for a Friend')}
        />
      </Grid>
    </Grid>
  );
