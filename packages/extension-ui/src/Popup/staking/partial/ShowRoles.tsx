...
export default function ShowRoles({ api, chain, label, mode, pool, style }: Props): React.ReactElement {
  ...
  const accountsToShow = useMemo(() => {
    ...
    if (mode === 'Roles') {
      return ([
        { address: pool.bondedPool?.roles?.root?.toString() ?? '', label: t('Root') },
        { address: pool.bondedPool?.roles?.depositor.toString() ?? '', label: t('Depositor') },
        { address: pool.bondedPool?.roles?.nominator?.toString() ?? '', label: t('Nominator') },
        { address: pool.bondedPool?.roles?.bouncer?.toString() ?? '', label: t('Bouncer') }
      ]);
    }
    ...
    return ([
      { address: pool.accounts?.stashId?.toString(), label: t('Stash id') },
      { address: pool.accounts?.rewardId?.toString() ?? '', label: t('Reward id') },
      ...(mayBeCommissionAddress ? [{ address: mayBeCommissionAddress?.toString() ?? 'N/A', label: t('Com. id') }] : [])
    ]);
  }, [mode, pool, t]);
