...
import { useAccountDisplay, useApi, useChain, useFormatted, useProxies, useTranslation } from '../../../../../hooks';
...
export default function RemoveValidators({ address, setShow, show, title }: Props): React.ReactElement {
  ...
  useEffect((): void => {
    ...
    chilled && chilled().paymentInfo(formatted as AddressOrPair).then((i) => setEstimatedFee(i?.partialFee)).catch(console.error);
  }, [api, chilled, formatted]);

  const remove = useCallback(async () => {
    try {
      if (!formatted || !api || !chilled || !chain) {
        return;
      }
