...
export default function Index(): React.ReactElement {
  ...
  const mayBeMyStashBalances = useBalances(stakingAccount?.stashId ? String(stakingAccount.stashId) : undefined);
  ...
  const hasEnoughDeposit = fastUnstakeDeposit && stakingConsts && myBalances && estimatedFee && getValue('available', myBalances)
    ? new BN(String(fastUnstakeDeposit)).add(estimatedFee).lt(getValue('available', myBalances) || BN_ZERO)
    : undefined;
  ...
  return (
    <Motion>
      ...
      <Grid container direction='column' m='20px auto' width='85%'>
        ...
        <Grid alignItems='center' container item lineHeight='28px' pl='5px' pt='15px'>
          ...
          <Typography fontSize='14px' fontWeight={300} lineHeight='inherit' pl='5px'>
            {t('Having {{deposit}} {{token}} available to deposit', { replace: { deposit: fastUnstakeDeposit && decimal ? amountToHuman(String(fastUnstakeDeposit), decimal) : '...', token } })}
          </Typography>
        </Grid>
      ...
      />
      {showFastUnstakeReview && formatted && api && getValue('available', balances) && chain && staked && !staked?.toBn()?.isZero() &&
        <FastUnstakeReview
          address={address}
          amount={staked?.toBn()}
          api={api}
