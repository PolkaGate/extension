...
export default function Index(): React.ReactElement {
  ...
  const staked = useMemo(() => stakingAccount ? stakingAccount.stakingLedger.active.toBn() : BN_ZERO, [stakingAccount]);
  const totalAfterStake = useMemo(() => decimal ? staked?.add(amountToMachine(amount, decimal)) : BN_ZERO, [amount, decimal, staked]);
  const isFirstTimeStaking = !!stakingAccount?.stakingLedger?.total?.toBn()?.isZero();
    ...
    <Motion>
      ...
      {validatorSelectionMethod === 'manual' && showSelectValidator && formatted &&
        <SelectValidators
          ...
          staked={stakingAccount?.stakingLedger?.active?.toBn() ?? BN_ZERO}
          stakingConsts={stakingConsts}
          stashId={formatted}
