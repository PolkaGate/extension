...
export default function Index(): React.ReactElement {
  ...
  const staked = useMemo(() => stakingAccount?.stakingLedger?.active?.toBn(), [stakingAccount?.stakingLedger?.active]);
  ...
  useEffect(() => {
    if (stakingAccount?.stakingLedger?.active?.toBn()?.isZero()) {
      setShake(true);
  ...
  return (
    <>
      ...
      <Container disableGutters sx={{ px: '15px' }}>
        ...
        <BouncingSubTitle circleStyle={{ m: '17px 0 0 149px' }} label={t('Solo Staking')} refresh={refresh} style={{ fontSize: '20px', fontWeight: 400 }} />
      ...
      <Grid container justifyContent='space-around' sx={{ borderTop: '2px solid', borderTopColor: 'secondary.main', bottom: 0, left: '4%', position: 'absolute', pt: '5px', pb: '2px', width: '92%' }}>
        ...
        <HorizontalMenuItem
          ...
          icon={
            <FontAwesomeIcon
              bounce={stakingAccount !== undefined && !stakingAccount?.nominators.length && !stakingAccount?.stakingLedger.active.toBn().isZero()} // do when has stake but does not nominations
              color={`${theme.palette.text.primary}`}
              icon={faHand}
              size='lg'
            />
          ...
          title={t('Validators')}
        />
        {stakingAccount?.stakingLedger?.total?.toBn()?.gt(BN_ZERO) &&
          <HorizontalMenuItem
            divider
            icon={
              <Box
