...
export default function Index (): React.ReactElement {
  ...
  return (
    <Motion>
      ...
      />
      <SubTitle
        label={t('Selected validators') + (nominatedValidatorsIds?.length ? ` (${nominatedValidatorsIds?.length})` : '')}
      />
      {nominatedValidatorsIds === null
        ? <>
          <Warn style={{ py: '15px' }} text={t('No validator found.')} />
      ...
      {nominatedValidatorsIds === null &&
        <>
          {!canNominate &&
            <Warn style={{ pt: '100px' }} text={t('Only pool owner can select validators.')} />
          }
          <PButton
