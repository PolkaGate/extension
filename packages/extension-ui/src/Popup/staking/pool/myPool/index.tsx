...
export default function Pool (): React.ReactElement {
  ...
  const POOL_ROLES = useMemo(() => ([
    ...
    { text: t('Depositor'), value: 'depositor' },
    { text: t('Root'), value: 'root' },
    { text: t('Nominator'), value: 'nominator' },
    { text: t('Bouncer'), value: 'bouncer' }
  ]), [t]);
  ...
  const blockHelperText = t('The pool state will be changed to Blocked, and no member will be able to join and only some admin roles can remove members.');
  const destroyHelperText = t('No one can join and all members can be removed without permissions. Once in destroying state, it cannot be reverted to another state.');
  const unblockHelperText = t('The pool state will be changed to Open, and any member will be able to join the pool.');
        ...
        <Grid container justifyContent='center'>
          ...
          {canChangeState &&
            <Grid alignItems='center' bottom='10px' container justifyContent='space-between' m='auto' position='absolute' width='92%'>
              <ActionBtn disabled={poolState === 'Destroying' || (!poolRoot && !poolBouncer)} onClick={goDestroying} showDivider text={t('Destroy')}>
                <AutoDeleteIcon sx={{ color: poolState === 'Destroying' || (!poolRoot && !poolBouncer) ? 'action.disabledBackground' : 'text.primary', fontSize: '21px' }} />
              </ActionBtn>
              {poolState === 'Blocked'
                ? (<ActionBtn onClick={goUnlock} disabled={(!poolRoot && !poolBouncer)} showDivider text={t('Unblock')}>
                  <UnblockIcon sx={{ color: (!poolRoot && !poolBouncer) ? 'action.disabledBackground' : 'text.primary', fontSize: '18px' }} />
                </ActionBtn>)
                : (<ActionBtn disabled={poolState === 'Destroying' || (!poolRoot && !poolBouncer)} onClick={goBlock} showDivider text={t('Block')}>
                  <BlockIcon sx={{ color: poolState === 'Destroying' || (!poolRoot && !poolBouncer) ? 'action.disabledBackground' : 'text.primary', fontSize: '21px' }} />
                </ActionBtn>)
              }
              <ActionBtn disabled={isRemoveAllDisabled || (!poolRoot && !poolBouncer)} onClick={goRemoveAll} showDivider text={t('Remove all')}>
              ...
              <ActionBtn disabled={poolState === 'Destroying' || !poolRoot} onClick={goEdit} text={t('Edit')}>
