...
export default function Index (): React.ReactElement {
  ...
  const maxUnlockingChunks = api && (api.consts.staking.maxUnlockingChunks as unknown as BN)?.toNumber() as unknown as number;
  ...
  useEffect(() => {
    ...
    if (isPoolDepositor && isPoolRoot && poolState !== 'Destroying' && partial.isZero()) {
      setHelperText(t('You need to change the pool state to Destroying first to be able to unstake.'));
    ...
    if (isPoolDepositor && isPoolRoot && poolState === 'Destroying' && poolMemberCounter !== 1 && partial.isZero()) {
      setHelperText(t('You need to remove all members first to be able to unstake.'));
    ...
    <Motion>
        ...
        <Grid container height='78px' justifyContent='center' m='auto' width='92%'>
          ...
          {helperButton &&
            <Button onClick={helperButton === 1 ? goToDestroying : goToRemoveAll}
              ...
              variant='text'
            >
              {helperButton === 1 ? t('Destroying') : t('RemoveAll')}
            </Button>}
        </Grid>
      }
      ...
      {!helperButton &&
        <Typography fontSize='16px' fontWeight={400} m='20px 0 0' textAlign='center'>
          {t('Your rewards wil be automatically withdrawn.')}
        </Typography>}
      <PButton
        _onClick={goToReview}
