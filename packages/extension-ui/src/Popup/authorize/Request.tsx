...
export default function Request({ authId, isFirst, request: { origin }, url }: Props): React.ReactElement<Props> {
  ...
  return (
    <>
      ...
      <Warning theme={theme}>
        <span style={{ overflowWrap: 'anywhere' }}>{t('An application, self-identifying as ')}<strong style={{ fontWeight: 400, textDecoration: 'underline' }}>{origin}</strong>{t(' is requesting access from ')}<strong style={{ fontWeight: 400, textDecoration: 'underline' }}>{url}</strong></span>
      </Warning>
      <Typography
        fontSize='14px'
        fontWeight={300}
