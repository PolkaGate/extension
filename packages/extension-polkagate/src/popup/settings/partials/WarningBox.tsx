...
export default function WarningBox({ description, title }: Props): React.ReactElement {
  ...
  return (
    <Stack columnGap='10px' direction='row' sx={{ bgcolor: isDark ? '#05091C' : '#FFF8EA', border: isDark ? 'none' : '1px solid #F8DCA0', borderRadius: '14px', mb: '5px', overflow: 'hidden', p: '15px', position: 'relative' }}>
      ...
      <Stack alignContent='flex-start' direction='column' justifyContent='start' width='100%'>
        <Typography color='text.primary' lineHeight='19.94px' textAlign='start' variant='H-4'>
          {title}
        </Typography>
