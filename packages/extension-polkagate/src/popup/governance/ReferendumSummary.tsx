  ...
  return (
    <Grid item key={key} onClick={!openDecisionDeposit ? onClick : () => null} sx={{ bgcolor: 'background.paper', borderRadius: '10px', cursor: 'pointer', height: '109px', p: '0 20px', my: '13px', '&:hover': { boxShadow: '0px 0px 10px rgba(0, 0, 0, 0.2)' } }}>
      <Grid container item sx={{ height: '30px' }}>
        {isThisMine &&
          <Grid item sx={{ bgcolor: 'label.main', color: 'text.secondary', fontSize: '12px', height: '20px', mr: '15px', textAlign: 'center', width: '85px' }}>
            {t('Mine')}
          </Grid>
        }
        {amIVoted &&
          <Grid item sx={{ bgcolor: 'label.main', color: 'text.secondary', fontSize: '12px', height: '20px', textAlign: 'center', width: '85px' }}>
            {t('Voted')}
          </Grid>
        }
      </Grid>
      ...
      <Grid alignItems='center' container item justifyContent='space-between'>
        ...
        <Grid item sx={{ textAlign: 'center', mb: '10px', color: 'white', fontSize: '17px', fontWeight: 400, border: '1px solid primary.main', borderRadius: '30px', bgcolor: STATUS_COLOR[refSummary.status], p: '5px 15px' }} xs={1.5}>
