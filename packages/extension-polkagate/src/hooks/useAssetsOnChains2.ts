...
export default function useAssetsOnChains2 (addresses: string[] | undefined): SavedAssets | undefined {
  ...
  const combineAndSetAssets = useCallback((assets: Assets) => {
    ...
    getStorage('assets2').then((res) => {
      const savedAssets = (res || DEFAULT_SAVED_ASSETS) as SavedAssets;

      console.log('assets2:', savedAssets);
      ...
      setStorage('assets', savedAssets).catch(console.error);
    }).catch(console.error);
  }, [addresses]);
  ...
  useEffect(() => {
    ...
    selectedChains?.forEach((genesisHash) => {
      const isSingleTokenChain = otherSingleAssetChains.find((o) => o.genesisHash === genesisHash)?.chain;
      ...
      if (ASSET_HUBS.includes(genesisHash)) { /** Checking assets balances on Asset Hub chains */
        ...
        const assetsToBeFetched = assetsChains[chainName]; /** we only fetch asset hubs assets which is whitelisted via Polkagate/apps-config */
      ...
      /** Checking assets balances on chains with more than one native asset */
    });
  }, [addresses, fetchAssetOnAssetHubs, fetchAssetOnRelayChain, isOutDated, selectedChains, workerCalled]);
