...
async function getNFTs (addresses) {
  ...
  const apiPromises = chainNames.map(async ([chainName, { name, prefix }]) => {
    ...
    return ({ api, chainName, formattedAddresses, originalAddresses: addresses, webSocket });
  });
...
export default async function getNftHandler (addresses, port) {
  ...
  for (let tryCount = 1; tryCount <= 5; tryCount++) {
    try {
      ...
      console.error(`Shared worker, getNFTs: Error while fetching NFTs on asset hubs, ${5 - tryCount} times to retry`, error);
