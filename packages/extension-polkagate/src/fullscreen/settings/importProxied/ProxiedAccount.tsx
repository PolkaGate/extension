...
import { getSubstrateAddress, toShortAddress, updateStorage } from '@polkadot/extension-polkagate/src/util';
...
const PROXIED_PROFILE_NAME = 'Proxied';
...
const addProxied = async(address: string, genesisHash: HexString) => {
    ...
    return Promise.all([
        ...
        updateMeta(address, PROXIED_PROFILE_NAME)
    ]);
};
...
function ProxiedAccount({ closePopup, mode = 'check' }: Props): React.ReactElement | null {
    ...
    const importProxiedAddresses = useProxiedAccounts(
    ...
    const selectableProxiedAddresses = useMemo<SelectableProxied[] | undefined>(() => {
        ...
        if (isImportMode) {
            if (!importProxiedAddresses) {
                return undefined;
            }

            return importProxiedAddresses
                .filter((addr) => {
    ...
    }, [accounts, allFoundProxiedAccounts, importProxiedAddresses, isImportMode]);
