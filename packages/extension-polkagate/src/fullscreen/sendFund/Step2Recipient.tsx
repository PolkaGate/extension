...
import { extractRelayChainName, migratedRelayNames } from '@polkadot/extension-polkagate/src/util/migrateHubUtils';
...
export default function Step2Recipient({ assetId, genesisHash, inputs, setInputs, teleportState }: Props): React.ReactElement {
  ...
  const chainOptions = useMemo((): DropdownOption[] => {
    ...
    try {
      ...
      return destinations
        .filter(({ text }) => !migratedRelayNames.includes(text.toLowerCase())) // remove migrated relays
        .map((option) => ({
          ...option,
          text: extractRelayChainName(option.text, true) ?? option.text // adjust migrated hub names
        }));
    } catch (error) {
